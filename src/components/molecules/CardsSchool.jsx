    ...
    <div className="grid grid-cols-1 mx-12 md:grid-cols-2 xl:grid-cols-4 grid-flow-row gap-6">
      ...
      {!isLoading &&
        ...
        dataSekolah.data.map((sekolah, index) => (
            <CardSchool key={sekolah.idsekolah} img="images/KakSetoSchool.png" title={sekolah.namasekolah} description={sekolah.description} link={sekolah.link} animate={animatecardschool[index]} index={index} />
        ))
      }
      </div>
  ) 
